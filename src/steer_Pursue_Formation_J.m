function [Boids, BoidsIndex]  = steer_Pursue_Formation_J(Target, Boids, Leader, D_Behind, D_Beside,...
                    Alpha_Horizontal, NumberBoidsOnTop, NumberBoidsOnBranch, NumberBoidsOnLeftBranch)
global BoidsNum;
...
WeightsMatrix = WeightFunction_Pursue_Formations(Target, Boids, BoidsNum, Alpha_Horizontal, D_Behind, D_Beside);
...
[LeaderRight, Boids] = Move_Pursue_Formations(Target, Weight, Boids, LeaderRight, Leader,RightBesideLeader, MinRightIndex);
...
[LeaderLeft, Boids] = Move_Pursue_Formations(Target, Weight, Boids, LeaderLeft, Leader, LeftBesideLeader, MinLeftIndex);

while (sum(BoidsFlag) < BoidsNum)        
    BoidIndex = sum(BoidsFlag) + 1;
    
    [RightBesideLeader, ~] = FindBesideLeader(LeaderRight, Alpha_Horizontal, D_Behind + 20, D_Beside + 30);
    [NearestRight, BoidsFlag, MinRightIndex] = NearestBoid(RightBesideLeader, BoidsFlag);
     BoidsIndex(BoidIndex) =  MinRightIndex;  
    Weight = WeightsMatrix(MinRightIndex);
    [NearestRight, Boids] = Move_Pursue_Formations(Target, Weight, Boids, NearestRight,LeaderRight, RightBesideLeader, MinRightIndex);
    LeaderRight = NearestRight;
    
    if (BoidIndex < NumberBoidsOnLeftBranch + NumberBoidsOnTop + 2*NumberBoidsOnBranch)
        [~, LeftBesideLeader] = FindBesideLeader(LeaderLeft, Alpha_Horizontal, D_Behind + 20, D_Beside - 50);
        [NearestLeft,BoidsFlag,MinLeftIndex] = NearestBoid(LeftBesideLeader, BoidsFlag);
        BoidsIndex(BoidIndex + 1) =  MinLeftIndex;
        Weight = WeightsMatrix(MinLeftIndex);
        [NearestLeft, Boids] = Move_Pursue_Formations(Target, Weight, Boids, NearestLeft,LeaderLeft, LeftBesideLeader, MinLeftIndex);

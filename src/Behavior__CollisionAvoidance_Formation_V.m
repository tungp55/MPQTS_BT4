function Behavior__CollisionAvoidance_Formation_V()
%% global variables
global TimeSteps;
global BoidsNum;
global Boids;
...
titleStr = 'Implementation of V-Formation and Collision Avoidance behavior';
...
while (timeTick < TimeSteps)   
    ...
    Leader = Boids(1, :);      
    Alpha_Horizontal = CalculationHorizontalAngle(Leader);        
    [Boids, BoidsIndex] = steer_CollisionAvoidance_Formation_V(Boids, Leader, ...
...
%% Record video
MyMovie = VideoWriter('V-Formation_CollisionAvoidance_Behavior.avi');
open(MyMovie);

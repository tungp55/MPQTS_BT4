function Behavior__Flee_Formation_U()
%% global variables
global TimeSteps;
global BoidsNum;
global Boids;
...
titleStr = 'Implementation of U-Formation and Pursue behavior';
...
while (timeTick < TimeSteps)
   ...
   force = steer_pursue(Boids(1,:), MousePosition);
    ...
    Leader = Boids(1, :);      
    Alpha_Horizontal = CalculationHorizontalAngle(Leader);        
    [Boids, BoidsIndex] = steer_Pursue_Formation_U(MousePosition, Boids, Leader, ...
...
%% Record video
MyMovie = VideoWriter('U-Formation_PursueBehavior.avi');
open(MyMovie);

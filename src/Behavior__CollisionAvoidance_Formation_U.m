function Behavior__CollisionAvoidance_Formation_U()
%% global variables
global TimeSteps;
global BoidsNum;
global Boids;
...
titleStr = 'Implementation of U-Formation and Collision Avoidance behavior';
...
while (timeTick < TimeSteps)          
    ...
    Leader = Boids(1, :);
    Alpha_Horizontal = CalculationHorizontalAngle(Leader);
    [Boids, BoidsIndex] = steer_CollisionAvoidance_Formation_U(Boids, Leader, ...
...
%% Record video
MyMovie = VideoWriter('U-Formation_CollisionAvoidanceBehavior.avi');
open(MyMovie);

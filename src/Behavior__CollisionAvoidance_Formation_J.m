function Behavior__CollisionAvoidance_Formation_J
global TimeSteps;
global BoidsNum;
global Boids;
global Targets;
...
titleStr = 'Implementation of J-Formation and Collision Avoidance behavior';
...
while (timeTick < TimeSteps) 
   ...
   force = 0.1*steer_arrival(Boids(1,:), RandomPosition); 
    ...
    Leader = Boids(1, :);      
    Alpha_Horizontal = CalculationHorizontalAngle(Leader);        
    [Boids, BoidsIndex] = steer_CollisionAvoidance_Formation_J(Boids, Leader, ...
...
%% Record video
MyMovie = VideoWriter('J-Formation_CollisionAvoidanceBehavior.avi');
open(MyMovie);
